...
async function tranformImage(path: string, width?: number , height?: number) {
    ...
    const cached = config.cachePath + path.substring(0, path.length - 4) + "_" + width + "_" + height + path.substring(path.length - 4);
...
async function fetchImage(path: string) {
    if (path === "/") {
        ...
        if (await Bun.file(config.imagePath + path).exists()) return Bun.file(config.imagePath + path);
        else {
...
export { tranformImage, fetchImage}